...
| P1-22 | Conta de backup pra SJC (failover): backup só entra quando a primária cai; nunca trava por nossa causa | Ops/Backend | **código done** (14/08) — cadeia [sp, sp2] no ar. ⚠️ **`sp2` DESATIVADA por decisão do José em 26/08** (senha do Luan desatualizada, não será renovada): SJC roda **sem failover** pela `sp`. Ver RUNBOOK. |
...
| P1-42 | Rate limit do login é contornável trocando um header (`X-Forwarded-For`): 100 tentativas, zero 429 — furo no P1-5 | Segurança | **done** (28/08) — IP do socket + balde por usuário; 13 testes, 5 deles por HTTP real |
