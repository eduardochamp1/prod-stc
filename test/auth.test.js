...
// passar a ler do banco (SPEC-gestao-usuarios). Eles são sobre o PARSING do
// formato do `.env`, então passaram a falar com `_usuariosDoEnv()`, que é essa
// parte e segue síncrona. A cobertura é a mesma; mudou o nome do que ela cobre.
//
// Sem essa troca, os 4 `assert.throws(() => getUsers(), ...)` falhariam com
// "Missing expected exception" — `assert.throws` não captura promise rejeitada.
const { test } = require('node:test');
const assert = require('node:assert');

// Stub do .env antes de require — _usuariosDoEnv lê de process.env.AUTH_USERS
...
const { _usuariosDoEnv } = require('../middleware/auth');

test('_usuariosDoEnv: parsing valido GUA|CAC', () => {
  withAuthUsers('engelmig_es:hashabc:user:GUA|CAC', () => {
    const users = _usuariosDoEnv();
...
test('_usuariosDoEnv: multiplas entradas (admin + ES)', () => {
...
test('_usuariosDoEnv: sigla unica', () => {
  withAuthUsers('guarapari:h:user:GUA', () => {
    assert.deepEqual(_usuariosDoEnv()[0].regionals, ['GUA']);
  });
});

test('_usuariosDoEnv: ALL rejeita com erro claro', () => {
  withAuthUsers('admin:h:admin:ALL', () => {
    assert.throws(() => _usuariosDoEnv(), /ALL.*nao e mais aceito|não é mais aceito/i);
  });
});

test('_usuariosDoEnv: ES rejeita com erro claro', () => {
  withAuthUsers('engelmig_es:h:user:ES', () => {
    assert.throws(() => _usuariosDoEnv(), /ES.*grupos|grupos.*aceitos/i);
  });
});

test('_usuariosDoEnv: sigla invalida rejeita', () => {
  withAuthUsers('x:h:user:XYZ', () => {
    assert.throws(() => _usuariosDoEnv(), /invalid|XYZ/i);
  });
});

test('_usuariosDoEnv: regional vazio rejeita', () => {
  withAuthUsers('x:h:user:', () => {
    assert.throws(() => _usuariosDoEnv(), /vazia|sem regionals/i);
  });
});

test('_usuariosDoEnv: case-insensitive normaliza pra uppercase', () => {
  withAuthUsers('x:h:user:gua|cac', () => {
    assert.deepEqual(_usuariosDoEnv()[0].regionals, ['GUA', 'CAC']);
  });
});

test('_usuariosDoEnv: AUTH_USERS vazio retorna []', () => {
  withAuthUsers('', () => {
    assert.deepEqual(_usuariosDoEnv(), []);
  });
});

const { login, verifyToken } = require('../middleware/auth');

// 21/09/2026: `login` virou ASSÍNCRONO ao passar a ler do banco. Aqui o `await`
// é atualização legítima — a função mudou de verdade, não o teste.
// A conta continua vindo do .env, então este caso não depende de banco.
test('login: payload contem v:2 e regionals (array)', async () => {
  process.env.AUTH_USERS = 'admin:' + require('crypto').createHash('sha256').update('senha').digest('hex') + ':admin:GUA|CAC|SJC';
  process.env.JWT_SECRET = 'test-secret';
  const result = await login('admin', 'senha');
  assert.ok(result);
